...
# Switch Hooked to Pin 7, LED to Pin 11
...
#We start by NOT taking time lapse pics
tlapsestate = False
  ...
  try:
    ...
    while True:
      ...
      if (gpsd.fix.longitude<>0) and (gpsd.fix.longitude<>'nan'):
          ...
          btnstate = GPIO.input(7)
	  if btnstate = True:
		#On the 2nd button push, end the time lapse and exit
		if tlapsestate = True

	  #Take a Pic if in tlapsestate=True
